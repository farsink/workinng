...
7. **Logging Multiple Sessions (Breaks)**
   - User logs a morning session (09:00 - 12:00) with specific notes.
   - User takes a break and logs an afternoon session (13:00 - 17:00).
   - User adds a late session (18:00 - 19:00) for overtime or follow-up tasks.
   - Each session maintains its own specific task notes. [web:83]
...
**User Story:**  
As a user, I want my data to stay coherent even if I edit the same session on two devices, so I don’t lose important changes.

**Acceptance Criteria:**
- If the same session (matching `id`) is modified on two devices before sync:
...
### 5.4 Breaks and Multiple Sessions

**User Story:**  
As a user, I want to log multiple distinct work sessions in a single day (with gaps for breaks) and have specific notes for each session.

**Acceptance Criteria:**
- User can add multiple entries for the same date.
- Each entry has its own `startTime`, `endTime`, and `tasks/notes`.
- The UI (Daily Summary) aggregates total duration from all sessions for that day.
- The UI (Timeline) displays sessions as distinct blocks on a 24-hour bar.
...
- **Local Schema (IndexedDB / Dexie):**
  - `TimeEntry` (represents a single work session):
    - `id: string` – globally unique ID (Dexie Cloud `@id` pattern).
    - `date: string` – ISO date (e.g., `2026-01-19`).
    - `startTime: number` – Start timestamp.
    - `endTime: number` – End timestamp.
    - `hoursWorked: number` – Calculated duration.
    - `tasks: string` – Per-session notes.
    - `isOvertime: boolean`.
    - `autoGenerated: boolean`.
    - `modifiedAt: number` (timestamp).
    - `userId: string` (implicit in realm/tenant if using Dexie Cloud).
